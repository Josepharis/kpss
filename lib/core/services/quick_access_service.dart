...
class QuickAccessService {
  static const String _baseCacheKey = 'quick_access_items';
  static const int _maxItems = 6; // Maximum number of quick access items

  static String get _cacheKey {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    if (uid == null) return _baseCacheKey;
    return '${_baseCacheKey}_$uid';
  }

  static Future<Map<String, int>> _loadCachedContentCounts(
    String topicId,
  ) async {
    try {
      ...
      final Map<String, dynamic> decoded =
          jsonDecode(contentCountsJson) as Map<String, dynamic>;
  ...
  static Future<void> addQuickAccessItem({
    ...
    try {
      ...
      if (items.length > _maxItems) {
        items.sort(
          (a, b) => b.lastAccessedTimestamp.compareTo(a.lastAccessedTimestamp),
        );
  ...
  static Future<bool> toggleQuickAccessItem({
    ...
    try {
      ...
      if (isInQuickAccess) {
        ...
        final resolvedPodcastCount = podcastCount > 0
            ? podcastCount
            : (cached['podcastCount'] ?? 0);
        final resolvedVideoCount = videoCount > 0
            ? videoCount
            : (cached['videoCount'] ?? 0);
        final resolvedFlashCardCount = flashCardCount > 0
            ? flashCardCount
            : (cached['flashCardCount'] ?? 0);
